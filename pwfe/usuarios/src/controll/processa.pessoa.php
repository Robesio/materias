...
/*
	if(!empty($_POST)){ //Se o verbo POST não estiver vazio
		$telefone = new Telefone();//Cria um novo objeto Pessoa (modelo)
		$telefone->setTelefone($_POST["telefone"]);//Preenche o modelo
		$sucesso = $pd->updateTel($telefone);
		if(!isset($sucesso["erro"])){
			http_response_code(201);
		}else{
			http_response_code(202);
...
*/
	if(!empty($_PUT)){ //Se o verbo PUT não estiver vazio
		...
		//echo $pd->update($pessoa);//Executa o método update de DAO passando o modelo como parâmetro
	}
