	...
	class PessoaDAO{//Classe DAO (Data Access Object) com os métodos CRUD
		...
		function update($pessoa){//Atualiza os dados no Banco de dados, nas duas tabelas pessoas e telefones
			...
			try{
				...
				if($status->execute()){
					if(($pessoa->getTelefone() != null) || ($pessoa->getTelefone() == "")){
						$SQL = "DELETE FROM telefones WHERE id_pessoa = $id";
						$resultCha = Conexao::getInstancia()->prepare($SQL);//Altera o telefone sem testar, esta lógica causa um erro de fluxo de dados
						
						if($resultCha->execute()){		
							$query = "INSERT INTO telefones VALUES ($id,'$telefone')";
							if(!Conexao::getInstancia()->exec($query))
								$resultado["erro"] = "Erro ao inserir registro do telefone".$query;	
						}else{
							$resultado["erro"] = "Erro ao deletar registro do telefone".$SQL;	
						}
					} 
				}
				$con = null;
